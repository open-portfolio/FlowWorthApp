...
extension WorthDocument {
    mutating func commitBuilderData(infoMessageStore: InfoMessageStore, timeZoneID: String) {
        do {
            ...
            displaySettings.pendingExcludedTxnMap.removeAll()
            ...
            infoMessageStore.add("Created snapshot #\(model.valuationSnapshots.count) at time stamp ‘\(formattedTimestamp)’", modelID: model.id)
            infoMessageStore.add("Ready to build next snapshot!", modelID: model.id)
            ...
            infoMessageStore.add(error.localizedDescription, modelID: model.id)
        }
    }
    ...
    mutating func clearBuilderDataAndRefresh(timeZoneID: String) {
        ...
        refreshContext(timeZoneID: timeZoneID)
    }
    ...
    mutating func refreshWorthResult(timeZoneID: String) { // investorEntitlement: Bool
        ...
        {
            mrCache = MatrixResultCache(ax: context,
            ...
            mrCache = MatrixResultCache(ax: context, timeZone: timeZone)
        }
        ...
        pendingSnapshot = PendingSnapshot(timestamp: displaySettings.builderCapturedAt,
    ...
    mutating func refreshContext(timeZoneID: String) {
        ...
        refreshWorthResult(timeZoneID: timeZoneID) // also resets mrCache & pendingSnapshot
